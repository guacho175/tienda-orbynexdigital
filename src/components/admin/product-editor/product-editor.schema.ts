...
export const productEditorSchema = z
  .object({
    ...
    slug: z
      .string()
      .trim()
      .min(2, productEditorConfig.copy.validation.slugRequired)
      .max(productEditorConfig.characterLimits.slug)
      .regex(/^[a-z0-9-]+$/, productEditorConfig.copy.validation.slugPattern),
    ...
    payment_external_enabled: z.boolean(),
  ...
  .superRefine((values, context) => {
    if (values.payment_external_enabled && !values.payment_url) {
      context.addIssue({
        code: z.ZodIssueCode.custom,
        path: ["payment_url"],
        message: "Ingresa la URL del pago externo o desactiva esta opcion.",
      });
    }
  });
